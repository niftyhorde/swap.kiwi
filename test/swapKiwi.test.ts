...
describe("Swap.Kiwi", async function () {
  ...
  describe("ERC721 tests", async function () {
    ...
    it('Should succesfully cancel swap by second user and emit "SwapCanceled" event', async function () {
      ...
      // check if NFTs are returned to their inital owners
      expect(await testNFT.ownerOf(60)).to.be.deep.equal(await signers[1].getAddress());
    });
    ...
    it('Should succesfully reject swap by swap initiator and emit "swapRejected" event', async function () {
      ...
      // check if NFTs are returned to their inital owners
      expect(await testNFT.ownerOf(80)).to.be.deep.equal(await signers[1].getAddress());
      expect(await testNFT.ownerOf(81)).to.be.deep.equal(await signers[1].getAddress());
    });
  ...
  describe("CryptoPunks tests", async function () {
    it('Should succesfully cancel CryptoPunks swap by second user and emit "SwapCanceled" event', async function () {
    ...
    it('Should succesfully reject CryptoPunks swap by second user and emit "swapRejected" event', async function () {
      // check that cryptoPunks1Signer is CryptoPunk with id=1 owner
      expect(await cryptoPunks.punkIndexToAddress(1)).to.be.deep.equal(
        await cryptoPunks1Signer.getAddress()
      );
      ...
      // first user NFT minting and swap deposit into SwapKiwi
      await testNFT.mint(await signers[1].getAddress(), 95);
      await testNFT.approve(swapKiwi.address, 95);
      const initiateSwapTx = await otherAppUser.initiateSwap(swapIdFromLogs, [testNFT.address], [95], {
        value: VALID_APP_FEE
      });
      const initiateSwapTxReceipt = await initiateSwapTx.wait(1);
      const initiateSwapLogs = await getEventWithArgsFromLogs(initiateSwapTxReceipt, "SwapInitiated");
      ...
      expect(initiateSwapLogs[0].args.to).to.be.deep.equal(await signers[1].getAddress());
      // check that second user NFTs are deposited into SwapKiwi
      expect(await testNFT.ownerOf(95)).to.be.deep.equal(swapKiwi.address);
      ...
      // check if all values are emitted in event
      expect(cancelTxlogs[0].eventName).to.be.deep.equal("SwapRejected");
      expect(cancelTxlogs[0].args.rejectedBy).to.be.deep.equal(await signers[1].getAddress());
      // check if NFTs are returned to their inital owners
      expect(await cryptoPunks.punkIndexToAddress(1)).to.be.deep.equal(
        await cryptoPunks1Signer.getAddress()
      );
      expect(await testNFT.ownerOf(95)).to.be.deep.equal(await signers[1].getAddress());
    });
    ...
    it("Should successfully swap CryptoPunks", async function () {
      ...
      // first user NFT minting and swap deposit into SwapKiwi
      await testNFT.mint(await signers[1].getAddress(), 101);
      await testNFT.approve(swapKiwi.address, 101);
      const initiateSwapTx = await otherAppUser.initiateSwap(swapIdFromLogs, [testNFT.address], [101], {
        value: VALID_APP_FEE
      });
      const initiateSwapTxReceipt = await initiateSwapTx.wait(1);
      const initiateSwapLogs = await getEventWithArgsFromLogs(initiateSwapTxReceipt, "SwapInitiated");
      ...
      expect(initiateSwapLogs[0].args.to).to.be.deep.equal(await signers[1].getAddress());
      // check that second user NFTs are deposited into SwapKiwi
      expect(await testNFT.ownerOf(101)).to.be.deep.equal(swapKiwi.address);
      ...
      expect(acceptSwapLogs[0].args.from).to.be.deep.equal(await signers[1].getAddress());
      expect(acceptSwapLogs[0].args.to).to.be.deep.equal(await signers[1].getAddress());
      // check that NFTs are transfered from SwapKiwi to participants
      expect(await cryptoPunks.punkIndexToAddress(1)).to.be.deep.equal(await signers[1].getAddress());
      expect(await testNFT.ownerOf(101)).to.be.deep.equal(await cryptoPunks1Signer.getAddress());
    });
  });
});
